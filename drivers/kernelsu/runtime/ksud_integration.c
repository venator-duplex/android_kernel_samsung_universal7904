				...
				struct user_arg_ptr *envp, int *flags)
{
	...
	if (unlikely(!memcmp(filename->name, system_bin_init,
		...
		if (!init_second_stage_executed &&
			...
			/*
			 * The exec never returns to the old init image.  Applying the
			 * policy from deferred task_work therefore races the first
			 * post-fs-data action.  Initialize it synchronously while the
			 * second-stage init process is still executing.
			 */
